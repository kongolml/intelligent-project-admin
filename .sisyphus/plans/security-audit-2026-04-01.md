...
| Add MongoDB TLS enforcement | MongoDB connections should use TLS to prevent MITM attacks | Phase 3.5 |
...
- [ ] 1.4 Add npm audit step to CI pipeline
  ...
  - Add `- run: npm audit --audit-level=critical` after `npm ci`
  - **NOTE**: Frontend plan also adds npm audit — align on `--audit-level=critical` for consistency
...
- [ ] 3.2 **FUTURE WORK**: Review S3 credential rotation strategy
  ...
  - **NOTE**: This is a long-term operational task, not a quick fix. Deferred to future phase.
...
- [ ] 3.6 **ADDED**: Verify frontend webhook secret matches
  - Confirm `PAYLOAD_WEBHOOK_SECRET` in frontend `.env.local` matches the value in adm `.env.local`
  - Rationale: If they don't match, webhooks will always fail with 401
...
| npm audit level | `--audit-level=critical` (aligned with frontend) |
...
- 2026-04-01: Frontend webhook validation already exists — Adm only needs to ensure secret parity.
- 2026-04-01: S3 credential rotation (Phase 3.2) moved to future work — requires separate operational planning.
